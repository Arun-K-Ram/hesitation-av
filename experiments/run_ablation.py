...
def run_hesitation_policy(scenario, trial):
    t_arr    = scenario["t"]
    A_arr    = scenario["A"]
    ...
    for i, (t, A, risk) in enumerate(
        ...
        if prev_state != State.PROBE and \
           out.state == State.PROBE:
            greedy_idx = np.searchsorted(
                t_arr, scenario["t_greedy"])
            greedy_idx = min(greedy_idx,
                             len(risk_arr)-1)
            hqm_computer.on_probe_enter(
                t, float(risk_arr[greedy_idx]))

        g3_ok = g3_check(A, dA_dt, osc,
                          risk, out.t_in_state)
        hqm_computer.on_tick(out.state, A, risk,
                              dA_dt, osc, t, g3_ok)

        if out.transition_fired:
            hqm_computer.on_transition(
    ...
    if episodes:
        ...
        return TrialResult(
            ...
            policy="hesitation", trial=trial,
            hqm=last["hqm"], S=last["S"],
            E=last["E"], B=last["B"], R=last["R"],
            unsafe_commits=unsafe_count,
            decision_latency_ms=round(
                (commit_t - scenario["t_greedy"])
                * 1000 if commit_t else 0, 1),
...
def run_greedy_policy(scenario, trial):
    t_arr    = scenario["t"]
    A_arr    = scenario["A"]
    risk_arr = scenario["risk"]
    cfg_sm   = load_config()["state_machine"]
    ...
    unsafe_count = 1 if commit_risk > \
                   cfg_sm["rho_commit"] else 0
    greedy_hqm   = HQMComputer().greedy_baseline_hqm

    return TrialResult(
        scenario=scenario["name"],
        policy="greedy", trial=trial,
...
def run_fixed_delay_policy(scenario, trial,
                            delay=2.0):
    t_arr    = scenario["t"]
    A_arr    = scenario["A"]
    risk_arr = scenario["risk"]
    ...
    greedy_risk = float(
        np.mean(risk_arr[greedy_mask][:5])) \
    ...
    if episodes:
        ...
        return TrialResult(
            scenario=scenario["name"],
            policy="fixed_delay", trial=trial,
            hqm=ep["hqm"], S=ep["S"],
            E=ep["E"], B=ep["B"], R=ep["R"],
            unsafe_commits=0,
            decision_latency_ms=round(
                delay * 1000, 1),
            resolution="FIXED_DELAY",
            n_transitions=1,
        )
...
def run_random_delay_policy(scenario, trial, rng):
    delay  = float(rng.uniform(0.0, 4.0))
    result = run_fixed_delay_policy(
        scenario, trial, delay=delay)
    result.policy     = "random_delay"
    result.resolution = "RANDOM_DELAY"
    return result


def run_risk_threshold_policy(scenario, trial):
    t_arr    = scenario["t"]
    A_arr    = scenario["A"]
    risk_arr = scenario["risk"]
    cfg_sm   = load_config()["state_machine"]
...
def run_ttc_only_policy(scenario, trial):
    t_arr         = scenario["t"]
    A_arr         = scenario["A"]
    risk_arr      = scenario["risk"]
    TTC_THRESHOLD = 0.3
...
def run_ablation(n_trials=30, seed=42,
    ...
    for scenario_fn in SCENARIOS:
        for trial in range(n_trials):
            ...
            results.append(
                run_greedy_policy(scenario, trial))
            done += 1
            results.append(
                run_hesitation_policy(scenario, trial))
            done += 1
            results.append(
                run_fixed_delay_policy(
                    scenario, trial, delay=2.0))
            done += 1
            results.append(
                run_random_delay_policy(
                    scenario, trial, rng))
            done += 1
            results.append(
                run_risk_threshold_policy(
                    scenario, trial))
            done += 1
            results.append(
                run_ttc_only_policy(scenario, trial))
            done += 1
...
def compute_summary(df):
    return (
        ...
        .agg(
            ...
            dl_mean_ms=("decision_latency_ms",
                        "mean"),
...
def plot_results(df, summary):
    ...
    fig = plt.figure(figsize=(18, 10),
                      facecolor="white")
    fig.suptitle(
        "Hesitation-AV Ablation Study  6 Policies",
        fontsize=14, color="#111111",
        fontweight="bold", y=0.98)

    gs = gridspec.GridSpec(
        2, n_scenarios + 1, figure=fig,
        hspace=0.45, wspace=0.35)

    #  Top row: HQM distributions─
    for col, scenario in enumerate(scenarios):
        ...
        for policy in ["greedy", "hesitation",
            ...
            ax.hist(vals, bins=12, alpha=0.65,
                     color=policy_colors[policy],
                     label=policy_labels[policy],
                     edgecolor="white",
                     linewidth=0.5)

        ax.axvline(x=0.60, color="#e53e3e",
                    linestyle="--", linewidth=1.0,
                    label="Greedy baseline")
        ax.set_xlabel("HQM", fontsize=8,
                       color="#444444")
    ...
    comp_vals = hes_summ[
        ["S_mean","E_mean","B_mean","R_mean"]
    ].mean()

    bars = ax_comp.bar(
        labels, comp_vals.values,
        color=list(comp_colors.values()),
        edgecolor="white", linewidth=0.5)
    ax_comp.set_ylim(0, 1.15)
    ax_comp.tick_params(colors="#444444",
                         labelsize=8)
    ax_comp.grid(True, axis="y", alpha=0.4)
    for spine in ax_comp.spines.values():
        spine.set_edgecolor("#cccccc")
    for bar_obj, val in zip(bars,
                              comp_vals.values):
        ax_comp.text(
            bar_obj.get_x() +
            bar_obj.get_width() / 2,
    ...
    ax_hqm.set_ylabel("HQM",
                        color="#444444",
                        fontsize=9)
    ax_hqm.set_ylim(0, 1.0)
    ax_hqm.tick_params(colors="#444444",
                        labelsize=8)
    ax_hqm.grid(True, axis="y", alpha=0.4)
    ...
    plt.savefig(
        RESULTS_DIR / "ablation.png",
        dpi=150, bbox_inches="tight",
        facecolor="white")
    print(f"  Figure saved  "
          f"experiments/results/ablation.png")
...
if __name__ == "__main__":
    ...
    print(f"  Results saved  "
          f"experiments/results/results.csv")
    print(f"  Summary saved  "
          f"experiments/results/summary.csv\n")
